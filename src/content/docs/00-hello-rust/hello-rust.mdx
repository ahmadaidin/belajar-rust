...
> **Catatan tentang `Cargo.lock`:** File ini dibuat secara otomatis oleh `cargo` berdasarkan `[dependencies]` di `Cargo.toml`. Jangan diedit manual — biarkan `cargo` yang mengelolanya. Lebih detailnya akan dipelajari di bagian
...
> **Catatan:** Sebenarnya jika menggunakan editor seperti Zed, atau VSCode, pesan error akan ditampilkan langsung di editor. Tetapi anda perlu mengetahui command `cargo check`, agar tidak bergantung kepada editor tersebut jika editor tersebut bermasalah.
